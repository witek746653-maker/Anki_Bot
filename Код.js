...
// SYSTEM_PROMPT is loaded from prompts/prompt.js (generated by scripts/build_prompts.js)
// prompts/system_prompt.md is the canonical editable source. During deploy the build script
// generates prompts/prompt.js which defines `var SYSTEM_PROMPT = '...';` for GAS to use.
// Fallback: allow using Script Properties (key `SYSTEM_PROMPT`) for runtime edits.
if (typeof SYSTEM_PROMPT === 'undefined' || !SYSTEM_PROMPT) {
  ...
  if (promptFromProps) {
    ...
    var SYSTEM_PROMPT = '';
    Logger.log('Warning: SYSTEM_PROMPT not found. Run `node ./scripts/build_prompts.js` to generate prompts/prompt.js or set Script Property SYSTEM_PROMPT.');
  }
}
...
function callOpenAI(prompt, systemOverride = SYSTEM_PROMPT) {
  ...
  const rawCard = JSON.parse(JSON.parse(res.getContentText()).choices[0].message.content);
...
function validateCard(card) {
  ...
  if (!allowedNoteTypes.includes(card.noteType)) throw new Error("Некорректный noteType от модели.");
  if (!allowedLangs.includes(card.lang)) throw new Error("Некорректный lang от модели.");
  if (!allowedDifficulty.includes(card.difficulty)) throw new Error("Некорректный difficulty от модели.");
  if (!card.topic) throw new Error("Пустой topic в карточке.");
  if (!card.back) throw new Error("Пустой back в карточке.");
  if (!Array.isArray(card.hints) || card.hints.length !== 7) throw new Error("Hint должен содержать ровно 7 строк.");

  const extraLines = card.extra.split("\n").map(s => s.trim()).filter(Boolean);
  if (extraLines.length !== 2) throw new Error("Extra должен содержать ровно 2 строки.");

  const backLower = card.back.toLowerCase();
  if (extraLines.some(line => line.toLowerCase().includes(backLower))) {
    throw new Error("Extra не должен повторять изучаемую фразу из Back.");
  }
  if (extraLines.some(line => /^(hint|hints|front|back|topic|type)\s*:/i.test(line))) {
    throw new Error("Extra содержит служебные заголовки полей.");
  }
  ...
  expectedHintPrefixes.forEach((prefix, index) => {
    if (!card.hints[index] || !card.hints[index].startsWith(prefix)) {
      throw new Error(`Некорректная структура Hint: ожидался пункт "${prefix}".`);
    }
  });
}
