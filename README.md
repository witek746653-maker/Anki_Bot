...
Если хотите, я могу сейчас подготовить патч: (A) заменить чтение токенов на `PropertiesService`, (B) заменить callback_data на draftId + сохранение mapping в `CacheService` и (C) удалить секреты из локальных файлов. Напишите «сделай патч» и я внесу изменения.
